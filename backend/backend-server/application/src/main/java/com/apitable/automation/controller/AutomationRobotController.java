...
import com.apitable.automation.model.CreateRobotRO;
...
import com.apitable.automation.enums.AutomationException;
...
public class AutomationRobotController {
     ...
     * @return {@link ResponseData}
     */
    @PostResource(path = "/robots", requiredPermission = false)
    @Operation(summary = "Create automation robot")
    @ApiResponses(@ApiResponse(responseCode = "200", useReturnTypeSchema = true))
    public ResponseData<AutomationSimpleVO> createRobot(
        @RequestBody @Valid CreateRobotRO data
    ) {
        Long userId = SessionContext.getUserId();
        iPermissionService.checkPermissionBySessionOrShare(data.getResourceId(), null,
            NodePermission.EDIT_NODE,
            status -> ExceptionUtil.isTrue(status, PermissionException.NODE_OPERATION_DENIED));
        // Check robot count limit per resource
        List<AutomationRobotDto> existingRobots =
            iAutomationRobotService.getRobotListByResourceId(data.getResourceId());
        if (existingRobots.size() >= 30) {
            throw new BusinessException(AutomationException.DST_ROBOT_LIMIT);
        }
        // Generate robot ID and create
        ...
        AutomationRobotEntity entity = AutomationRobotEntity.builder()
            ...
            .updatedBy(userId)
            .build();
        iAutomationRobotService.create(entity);
        AutomationSimpleVO vo = AutomationSimpleVO.builder()
            .robotId(robotId)
            .resourceId(data.getResourceId())
            .name(data.getName())
            .description(data.getDescription())
