...
public class UserController {
    ...
    private String returnSpaceDomain(final String spaceId,
        ...
        if (StrUtil.isBlank(spaceId) && StrUtil.isBlank(userSpaceId)) {
    ...
    public ResponseData<Void> updatePwd(
        ...
        // ===== 旧密码验证（内部使用，无需邮件/短信验证码）=====
        // For internal deployment without email/SMS, verify identity via old password
        if (StrUtil.isNotBlank(param.getOldPassword())) {
            iUserService.verifyOldPassword(userId, param.getOldPassword());
            iUserService.updatePwd(userId, param.getPassword());
            return ResponseData.success();
        }

        // ===== 验证码验证（已注释，需要时取消注释）=====
        // For environments with email/SMS configured, use verification code
        ...
        // ===== 无密码用户初始化密码 =====
        // Account without password can set password without any verification
