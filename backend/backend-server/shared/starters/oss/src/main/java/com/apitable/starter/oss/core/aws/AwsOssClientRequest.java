...
public class AwsOssClientRequest extends AbstractOssClientRequest {
    ...
    public AwsOssClientRequest(AmazonS3 amazonClient, boolean autoCreateBucket) {
        this(amazonClient, autoCreateBucket, null);
    }
    ...
    protected void isBucketExist(String bucketName) {
        ...
        if (!existBucket) {
            if (autoCreateBucket) {
                ...
                String policy = StrUtil.isNotBlank(bucketPolicyJson)
                    ? bucketPolicyJson : getDefaultBucketPolicy(bucketName);
                try {
                    amazonClient.setBucketPolicy(
                        new SetBucketPolicyRequest(bucketName, policy));
                } catch (Exception e) {
                    LOGGER.error("Failed to set bucket policy for '{}': {}",
    ...
    private static String getDefaultBucketPolicy(String bucketName) {
