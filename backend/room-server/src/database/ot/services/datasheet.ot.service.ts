    ...
    if (resultSet.toCreateRecord.size || resultSet.toUnarchiveRecord.size) {
      ...
      for (const [recordId, value] of resultSet.toCreateRecord.entries()) {
        ...
        for (const fieldId in value) {
          ...
          if (!field) {
            this.logger.warn('Non-existent fields in data being written', {
              datasheetId,
              fieldId,
            });
            continue;
          ...
          if (Field.bindContext(field, {} as IReduxState).isComputed) {
            this.logger.warn('ot service: writting compute field failed', {
              datasheetId,
              field,
            });
            continue;
          ...
          if (error) {
            this.logger.warn('Format error for data being written', {
              datasheetId,
