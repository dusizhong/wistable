...
def build_extract_messages(fields: list[dict[str, Any]], markdown_chunk: str) -> list[dict[str, str]]:
    ...
    system = (
        ...
        '「统一社会信用代码」，字段「金额」对应「价税合计」「不含税金额」等），'
        '按语义匹配，不要因字面不同就漏掉字段。\n'
        '5. 仅使用给定的字段名作为 key，不要新增字段。\n'
        '6. SingleSelect 只能填「可选值」之一；MultiSelect 填可选值数组。\n'
        '7. Number/Currency 填纯数字（去掉单位、千分位、货币符号）；Checkbox 填 true/false。'
        '金额数值必须原样保留小数点与小数位：文档写 ¥334.92 就输出 334.92、写 8.08 就输出 8.08，'
        '绝不能去掉小数点写成 33492、808 这类被放大 100 倍的整数。\n'
        '8. 抽取不到任何内容时返回 {"records": []}。\n'
        f'字段定义：\n{hints}'
    )
    user = f'文档内容：\n{markdown_chunk}'
    return [
