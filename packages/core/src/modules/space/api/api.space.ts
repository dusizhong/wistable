...
export function getAdminUserList(params: { pageNo: number; pageSize: number; keyword?: string }) {
  return axios.get(Url.ADMIN_USER_LIST, { params });
}

export function adminCreateUser(data: { email: string; password: string; nickName?: string; mobile?: string; role?: string }) {
  return axios.post(Url.ADMIN_USER_CREATE, data);
}

export function adminUpdateUser(userId: string, data: { role?: string; nickName?: string; email?: string; mobile?: string }) {
  return axios.post(urlcat(Url.ADMIN_USER_UPDATE, { userId }), data);
}

export function adminSetUserPassword(userId: string, data: { password: string }) {
  return axios.post(urlcat(Url.ADMIN_USER_PASSWORD, { userId }), data);
}

export function adminToggleUser(userId: string) {
  return axios.post(urlcat(Url.ADMIN_USER_TOGGLE, { userId }));
}

export function adminDeleteUser(userId: string) {
  return axios.delete(urlcat(Url.ADMIN_USER_DELETE, { userId }));
}

export function getAdminSpaceList(params: { pageNo: number; pageSize: number; keyword?: string }) {
  return axios.get(Url.ADMIN_SPACE_LIST, { params });
}

export function adminCreateSpace(data: { name: string; ownerUserId?: string; ownerEmail?: string }) {
  return axios.post(Url.ADMIN_SPACE_CREATE, data);
}

export function adminUpdateSpace(spaceId: string, data: { name?: string; logo?: string }) {
  return axios.post(urlcat(Url.ADMIN_SPACE_UPDATE, { spaceId }), data);
}

export function adminDeleteSpace(spaceId: string) {
  return axios.delete(urlcat(Url.ADMIN_SPACE_DELETE, { spaceId }));
}

export function getAdminSpaceCapacity(spaceId: string) {
  return axios.get(urlcat(Url.ADMIN_SPACE_CAPACITY, { spaceId }));
}

export function updateAdminSpaceCapacity(spaceId: string, data: Record<string, number | null>) {
  return axios.post(urlcat(Url.ADMIN_SPACE_CAPACITY, { spaceId }), data);
}

export function getAdminNotificationList(params: { pageNo: number; pageSize: number; keyword?: string }) {
  return axios.get(Url.ADMIN_NOTIFICATION_LIST, { params });
}

export function getAdminNotificationDetail(id: number) {
  return axios.get(urlcat(Url.ADMIN_NOTIFICATION_DETAIL, { id }));
}

export function adminCreateNotification(data: { title: string; content: string; url?: string }) {
  return axios.post(Url.ADMIN_NOTIFICATION_CREATE, data);
}

export function adminUpdateNotification(id: number, data: { title: string; content: string; url?: string }) {
  return axios.post(urlcat(Url.ADMIN_NOTIFICATION_DETAIL, { id }), data);
}

export function adminDeleteNotification(id: number) {
  return axios.delete(urlcat(Url.ADMIN_NOTIFICATION_DETAIL, { id }));
}

export function adminSendNotification(id: number) {
  return axios.post(urlcat(Url.ADMIN_NOTIFICATION_SEND, { id }));
}

// feedback management
export function submitFeedback(data: { nickName: string; email: string; content: string }) {
  return axios.post<IApiWrapper>(Url.FEEDBACK_SUBMIT, data);
}

export function getAdminFeedbackList(params: { pageNo: number; pageSize: number; keyword?: string }) {
  return axios.get<IApiWrapper & { data: { records: any[]; total: number } }>(Url.ADMIN_FEEDBACK_LIST, { params });
}

export function adminProcessFeedback(id: number) {
  return axios.post<IApiWrapper>(urlcat(Url.ADMIN_FEEDBACK_PROCESS, { id }));
}
