...
export const getAdminNotificationList = (params: { pageNo: number; pageSize: number; keyword?: string }) => {
  return axios.get<IApiWrapper & { data: { records: any[]; total: number } }>(Url.ADMIN_NOTIFICATION_LIST, { params });
};

export const adminCreateNotification = (data: { title: string; content: string; url?: string }) => {
  return axios.post<IApiWrapper>(Url.ADMIN_NOTIFICATION_CREATE, data);
};

export const adminUpdateNotification = (id: string, data: { title: string; content: string; url?: string }) => {
  return axios.post<IApiWrapper>(urlcat(Url.ADMIN_NOTIFICATION_UPDATE, { id }), data);
};

export const adminDeleteNotification = (id: string) => {
  return axios.delete<IApiWrapper>(urlcat(Url.ADMIN_NOTIFICATION_DELETE, { id }));
};

export const adminSendNotification = (id: string) => {
  return axios.post<IApiWrapper>(urlcat(Url.ADMIN_NOTIFICATION_SEND, { id }));
};

// feedback management
export const submitFeedback = (data: { nickName: string; email: string; content: string }) => {
  return axios.post<IApiWrapper>(Url.FEEDBACK_SUBMIT, data);
};

export const getAdminFeedbackList = (params: { pageNo: number; pageSize: number; keyword?: string }) => {
  return axios.get<IApiWrapper & { data: { records: any[]; total: number } }>(Url.ADMIN_FEEDBACK_LIST, { params });
};

export const adminProcessFeedback = (id: number) => {
