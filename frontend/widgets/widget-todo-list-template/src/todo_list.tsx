...
export const TodoList: React.FC = () => {
  ...
  const addRecords = () => {
    ...
    const addRecordCheckResult =
      datasheet.checkPermissionsForAddRecord(fieldsMap);
  ...
  const setRecord = (recordId: string, fieldMap) => {
    ...
    const setRecordCheckResult = datasheet.checkPermissionsForSetRecord(
      recordId,
      fieldMap
    );
    ...
    return (
      ...
      >
        <input
          type="checkbox"
          id="check1"
          checked={curCellValue}
          onClick={() =>
            setRecord(record.recordId, { [fieldId]: !curCellValue })
          }
        />
  ...
  return (
    <div style={{ overflow: "auto", height: "100%" }}>
      <Box padding="16px 10px 10px" borderBottom="2px solid lightgrey">
        <FormItem label="View">
          <ViewPicker
            viewId={viewId}
