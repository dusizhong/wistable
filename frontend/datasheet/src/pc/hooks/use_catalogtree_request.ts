...
export const useCatalogTreeRequest = () => {
  ...
  const getCollaboratorListPageReq = async (pageNo: number, nodeId: string) => {
    const data = await getNodeRoleListReq(nodeId, true, false, '');
    if (!data) {
      return Promise.resolve({ pageSize: ConfigConstant.MEMBER_LIST_PAGE_SIZE, pageNo: 1, total: 0, records: [] });
    }
    const { owner, admins, members } = data;
    const recordList: any[] = [];
    if (owner) {
      recordList.push({ ...owner, isWorkbenchAdmin: false, isControlOwner: true });
    }
    admins?.forEach((m: any) => recordList.push({ ...m, isWorkbenchAdmin: true, isControlOwner: false }));
    members?.forEach((m: any) => recordList.push({ ...m, isWorkbenchAdmin: false, isControlOwner: false }));
    return { pageSize: ConfigConstant.MEMBER_LIST_PAGE_SIZE, pageNo: 1, total: recordList.length, records: recordList };
  };
