...
export const useUserRequest = () => {
  ...
  const signUpReq = (token: string) => {
    return Api.signUp(token).then((res) => {
  ...
  const registerReq = (username: string, credential: string) => {
    const defaultLang = (): string => 'zh-CN';
  ...
  const modifyPasswordReq = (password: string, oldPassword: string, code?: string, type?: string) => {
    return Api.updatePwd(password, code, type, oldPassword).then((res) => {
      return res.data;
    });
  };
