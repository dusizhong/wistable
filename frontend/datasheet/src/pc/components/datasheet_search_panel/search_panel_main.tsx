...
import { ButtonPlus, Loading } from '../common';
...
export const SearchPanelMain: React.FC<ISearchPanelProps> = (props) => {
  ...
  return (
    <div className={styles.searchPanel} onClick={(e) => e.stopPropagation()}>
      ...
      {!secondConfirmType && isPc && (
        <div style={{ padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--borderCommonDefault)' }}>
          <button
            type="button"
            style={{ padding: '6px 16px', background: 'transparent', border: '1px solid var(--borderCommonDefault)', borderRadius: '4px', cursor: 'pointer', fontSize: '14px', color: 'var(--textCommonPrimary)' }}
            onClick={hidePanel}
          >
            {t(Strings.cancel)}
          </button>
