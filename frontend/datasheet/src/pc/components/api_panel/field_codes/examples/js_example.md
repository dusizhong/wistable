...
```js
// 如果不能使用 es6 import，可用 const WisTable = require('apitable').default; 代替
import { WisTable } from "apitable";

const wistable = new WisTable({ token: "{{ apiToken }}", fieldKey: "{{ fieldKey }}" });
// 通过 datasheetId 来指定要从哪张表操作数据。
const datasheet = wistable.datasheet("{{ datasheetId }}");
```
...
new WisTable({
   ...
   * （选填）目标服务器地址
   */
  host: 'https://your-server.com/fusion/v1', // 默认值
});
...
datasheet.record.query({
  /**
   * （选填）视图ID。默认为表中第一个视图。请求会返回视图中经过视图中筛选/排序后的结果，可以搭配使用fields参数过滤不需要的字段数据
   */
  viewId: 'viewId1',
  /**
   * （选填）对指定表的记录进行排序。由多个"排序对象"组成的数组。支持顺序：'asc' 和 逆序：'desc'。注：此参数指定的排序条件将会覆盖视图里的排序条件。
   */
...
```json
{
  ...
  "data": {
    ...
    "width": 240,
    "height": 240,
    "url": "https://your-server.com/space/2020/09/22/01ee7202922d48688f61e34f12da5abc"
  }
}
```
