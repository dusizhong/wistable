...
export class RouterStrategy implements IRouterStrategy {
  ...
  static [Navigation.USER_ADMIN] = ({ query, clearQuery }: IFunctionParams): IFunctionResult => {
    return ['/user-admin', query, clearQuery];
  };

  static [Navigation.SPACE_ADMIN] = ({ query, clearQuery }: IFunctionParams): IFunctionResult => {
    return ['/space-admin', query, clearQuery];
  };

  static [Navigation.FEEDBACK_ADMIN] = ({ query, clearQuery }: IFunctionParams): IFunctionResult => {
    return ['/feedback-admin', query, clearQuery];
  };
