...
export const AutomationPanel: FC<{ onClose?: () => void; resourceId?: string, panel?: IAutomationPanel }> = memo(({ onClose, panel, resourceId }) => {
  ...
  const { initialize, createNewRobot } = useAutomationNavigateController();
  ...
  useEffect(() => {
    ...
    getResourceAutomations(resourceId, {
      ...
      .then(async (res) => {
        ...
        if (!firstItem) {
          // No robots exist for this resource, auto-create one
          await createNewRobot(resourceId);
          return;
        }
      ...
      .catch((e) => {
        console.error(e);
        Message.error({ content: t(Strings.robot_save_step_failed) });
      }).finally(()=> {
        setLoading(false);
      });
