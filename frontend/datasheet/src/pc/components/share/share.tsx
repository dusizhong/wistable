...
import { getAssetUrl, IShareInfo, Navigation, StoreActions, Strings, t } from '@apitable/core';
...
const Share: React.FC<React.PropsWithChildren<IShareProps>> = ({ shareInfo }) => {
  ...
  const configRouter = () => {
    ...
    setTimeout(() => {
       ...
       * @see https://github.com/apitable/apitable/issues/5795
       */
  ...
  const LightLogo = IS_AITABLE ? getAssetUrl(LONG_LIGHT_LOGO!) : undefined;
  const DarkLogo = IS_AITABLE ? getAssetUrl(LONG_DARK_LOGO!) : undefined;
  ...
  return (
    <ShareContext.Provider value={{ shareInfo: shareSpace }}>
      ...
      >
        ...
        {isIframe() && !formId && IS_AITABLE && LightLogo && DarkLogo && (
