...
interface IFormDataRenderProps {
    ...
    formData: any;
    disableRetrieveSchema?: boolean;
}

export const FormDataRender = (props: IFormDataRenderProps) => {
  const { nodeSchema, formData } = props;
  ...
  // Use schema properties for labels when available, fall back to plain key-value
  const schema = nodeSchema.schema as any;
  const properties = schema?.properties ?? {};
  const keys = Object.keys(properties).length > 0 ? Object.keys(properties) : Object.keys(formData);
  ...
  return (
    <Box marginTop="8px" marginBottom="16px" padding="0 16px" boxShadow={`inset 1px 0px 0px ${theme.color.fc5}`}
      ...
      {keys.map((key) => {
        const propertyValue = formData[key];
        const label = properties[key]?.title || key;
        return <KeyValueDisplay key={key} label={label} value={propertyValue}/>;
      })}
    </Box>
  );
};
