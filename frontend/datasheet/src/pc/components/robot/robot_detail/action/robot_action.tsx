...
import { Box, SearchSelect, Typography, useThemeColors } from '@apitable/components';
...
import { getFilterActionTypes, getNodeOutputSchemaList, getNodeTypeOptions } from '../../helper';
...
import { NodeFormInfo } from '../node_form';
...
import { SendWebhookForm, SendMailForm } from './forms';
...
export const RobotAction = memo((props: IRobotActionProps) => {
  ...
  const handleActionFormSubmit = (newFormData: any) => {
  ...
  // Action form component map — routes endpoint to fixed form component
  const actionFormMap: Record<string, React.FC<any>> = useMemo(() => ({
    sendRequest: SendWebhookForm,
    sendMail: SendMailForm,
  }), []);

  const isEntryMode = editType === EditType.entry;
  const ActionFormComponent = actionFormMap[actionType?.endpoint ?? ''];
  ...
  if (isEntryMode) {
    return (
      <NodeFormInfo
        ...
        handleClick={dataClick}
        description={actionType.description}
        formData={formData}
        serviceLogo={getAutomationServiceLogo(actionType.service)}
      >
        <SearchSelect
          disabled={!permissions.editable}
          clazz={{
          ...
          {ActionFormComponent ? (
            <ActionFormComponent
              ...
              onChange={(newFormData: any) => {
                setMap(produce((draft) => { draft.set(action.id!, newFormData); }));
              }}
      ...
      <Box flex="0 0 32px" marginTop="16px" display="flex" width="100%" justifyContent="center" flexDirection="row-reverse">
        <Box display="flex">
          <button
            ...
            onClick={() => handleActionFormSubmit(formData)}
          >
