...
import { Events, IReduxState, NAV_ID, Player, StoreActions, Strings, t } from '@apitable/core';
import {
  ChevronDownOutlined,
...
enum NavKey {
  ...
  NotificationAdmin = 'notify-admin',
...
export const Navigation: FC<React.PropsWithChildren<unknown>> = () => {
  ...
  const { user, unReadCount, newNoticeListFromWs } = useAppSelector(
    (state: IReduxState) => ({
  ...
  const navList = Player.applyFilters(Events.get_nav_list, [
    ...
    },
    {
      ...
      text: t(Strings.nav_space_statistics),
      key: NavKey.SpaceManagement,
      domId: NAV_ID.ICON_SPACE_MANAGE,
    },
    {
      routeAddress: '/user-admin' + search,
      icon: UserAdminOutlined,
      text: t(Strings.nav_user_admin),
      key: NavKey.UserAdmin,
      domId: 'icon-user-admin',
    },
    {
      routeAddress: '/space-admin' + search,
      icon: Setting2Outlined,
      text: t(Strings.nav_space_admin),
      key: NavKey.SpaceAdmin,
      domId: 'icon-space-admin',
    },
    {
      routeAddress: '/notify-admin' + search,
      icon: HornOutlined,
      text: t(Strings.nav_notification_admin),
      key: NavKey.NotificationAdmin,
      domId: 'icon-notification-admin',
    },
    {
      routeAddress: '/feedback-admin' + search,
      icon: AdviseOutlined,
      text: t(Strings.nav_feedback_admin),
      key: NavKey.FeedbackAdmin,
      domId: 'icon-feedback-admin',
    },
  ]);
      ...
      <div className={classNames(styles.navigation, templateActive && styles.templateActived, notice && styles.noticeOpend)}>
        <div className={styles.spaceLogo} onClick={openSpaceMenu}>
          <div className={styles.spaceImg}>
            <Avatar type={AvatarType.Space} title={user!.spaceName} id={user!.spaceId} src={'/logo.png'} size={AvatarSize.Size32} isGzip={false} />
          </div>
        ...
        <div className={styles.navWrapper} onClick={hiddenUserMenu}>
          {navList.map((item: any) => {
            ...
            if (item.key === NavKey.Template) return null; // HIDE_FEATURE: 模板中心
            if (item.key === NavKey.SpaceManagement) return null; // HIDE_FEATURE: 空间驾驶舱
            if (item.key === NavKey.NotificationAdmin) return null; // HIDE_FEATURE: 通知管理
            if (item.key === NavKey.FeedbackAdmin) return null; // HIDE_FEATURE: 反馈管理
            if (user && !user!.isAdmin && item.key === NavKey.SpaceManagement) {
              return null;
            }
            if (user && user.role !== 'admin' &&
                (item.key === NavKey.UserAdmin || item.key === NavKey.SpaceAdmin || item.key === NavKey.NotificationAdmin || item.key === NavKey.FeedbackAdmin)) {
              return null;
            }
        ...
        {showCreateModal && user.role === 'admin' && <CreateSpaceModal isMobile={isMobile} setShowCreateModal={setShowCreateModal} />}
      </div>
      <ComponentDisplay maxWidthCompatible={ScreenSize.md}>
        <Popup
